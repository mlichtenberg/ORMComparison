...
namespace ContosoUniversity.DataAccessNoSp
{
	partial class EnrollmentDAL : IEnrollmentDAL
	{
			...
			int enrollmentID )
		{
            ...
            // THESE TWO LINES REPLACED WITH THE FOLLOWING THREE LINES
            ...
            string sql = "SELECT [EnrollmentID], [CourseID], [StudentID], [Grade] FROM [dbo].[Enrollment] WHERE [EnrollmentID] = @EnrollmentID";
            using (SqlCommand command = CustomSqlHelper.CreateCommand(sql, connection, transaction, CommandType.Text,
                CustomSqlHelper.CreateInputParameter("EnrollmentID", SqlDbType.Int, null, false, enrollmentID)))
            {
			...
			int? grade)
		{
            ...
            // REPLACE THESE SIX LINES WITH THE FOLLOWING SEVEN LINES
            ...
            string sql = "INSERT INTO [dbo].[Enrollment] ([CourseID], [StudentID], [Grade]) VALUES (@CourseID, @StudentID, @Grade); " +
                        "SELECT [EnrollmentID], [CourseID], [StudentID], [Grade] FROM[dbo].[Enrollment] WHERE [EnrollmentID] = Scope_Identity()";
			...
			int enrollmentID)
		{
            ...
            // THESE THREE ROWS REPLACED WITH THE FOLLOWING THREE ROWS
            //using (SqlCommand command = CustomSqlHelper.CreateCommand("EnrollmentDeleteAuto", connection, transaction,
            //     CustomSqlHelper.CreateInputParameter("EnrollmentID", SqlDbType.Int, null, false, enrollmentID),
            //         CustomSqlHelper.CreateReturnValueParameter("ReturnCode", SqlDbType.Int, null, false)))

            string sql = "DELETE FROM [dbo].[Enrollment] WHERE [EnrollmentID] = @EnrollmentID";

			using (SqlCommand command = CustomSqlHelper.CreateCommand(sql, connection, transaction, CommandType.Text,
				CustomSqlHelper.CreateInputParameter("EnrollmentID", SqlDbType.Int, null, false, enrollmentID)))
			{
                // THIS ROW REPLACED WITH THE FOLLOWING ROW
				//int returnCode = CustomSqlHelper.ExecuteNonQuery(command, "ReturnCode");
                int returnCode = CustomSqlHelper.ExecuteNonQuery(command);
			...
			int? grade)
		{
            ...
            string sql = "UPDATE [dbo].[Enrollment] SET [CourseID] = @CourseID, [StudentID] = @StudentID, [Grade] = @Grade WHERE [EnrollmentID] = @EnrollmentID;" +
                        "SELECT [EnrollmentID], [CourseID], [StudentID], [Grade] FROM[dbo].[Enrollment] WHERE [EnrollmentID] = @EnrollmentID";
